...
# 配置数据（与update_people_data.py保持一致）
GENDERS = ['男', '女']
# 经常出国的职业（20个）
...
def generate_work_history(age, occupation):
    ...
    for i in range(work_count):
        ...
        if occupation in ['工程师', '医生', '教师'] and i > 0:
            position = random.choice(['软件工程师', '产品经理', '数据分析师', '项目经理', '销售经理', '市场专员'])
...
def generate_visit_records(region):
    ...
    for i in range(visit_count):
        ...
        visit_province = REGION_TO_PROVINCE.get(visit_city, random.choice(list(PROVINCES.keys())))
...
def add_recent_people():
    ...
    try:
        ...
        # 生成今天新增的人员（约100条）
        print(f'\n生成今天新增的人员数据（{today_start.strftime("%Y-%m-%d")}）...')
        for i in range(100):
            try:
                # 随机生成今天的时间
                random_seconds = random.randint(0, int((today_end - today_start).total_seconds()))
                created_time = today_start + timedelta(seconds=random_seconds)
                last_update_time = created_time + timedelta(hours=random.randint(0, int((now - created_time).total_seconds() / 3600)))
                ...
                province = REGION_TO_PROVINCE.get(region, random.choice(list(PROVINCES.keys())))
                
                # 生成扩展信息
                tags = generate_tags(occupation, region)
                education_history = generate_education_history(age)
                work_history = generate_work_history(age, occupation)
                social_media = generate_social_media(age)
                visit_records = generate_visit_records(region)
                flight_records = generate_flight_records(region)
                train_records = generate_train_records(region)
                ...
                person_data = {
                    ...
                    'lastUpdate': last_update_time.strftime('%Y-%m-%d %H:%M:%S'),
                    'gender': gender,
                ...
                # 创建人员（注意：created_at会在数据库层面自动设置，但我们需要手动设置）
                # 由于create_person方法可能不支持直接设置created_at，我们需要使用SQL直接插入
                ...
                cursor.execute('''
                    INSERT INTO people (
                        ...
                        visit_records, flight_records, train_records, hometown, nationality, visa_type, institution, created_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    person_data['name'],
                    person_data['idCard'],
                    ...
                    json.dumps(person_data['tags'], ensure_ascii=False),
                    json.dumps(person_data['educationHistory'], ensure_ascii=False),
                    json.dumps(person_data['workHistory'], ensure_ascii=False),
                    json.dumps(person_data['socialMedia'], ensure_ascii=False),
                    json.dumps(person_data['visitRecords'], ensure_ascii=False),
                    json.dumps(person_data['flightRecords'], ensure_ascii=False),
                    json.dumps(person_data['trainRecords'], ensure_ascii=False),
                    person_data['hometown'],
                    '',  # nationality字段（数据库有但前端不使用）
                    person_data['visaType'],
                    person_data['institution'],
                    created_time.strftime('%Y-%m-%d %H:%M:%S')  # 设置创建时间为今天
                ))
                db.connection.commit()
                added_count += 1
                
                if added_count % 20 == 0:
                    print(f'已添加 {added_count} 条今天的数据...')
                    
            except Exception as e:
                print(f'添加第 {i+1} 条今天数据失败: {e}')
                continue
        
        # 生成昨天新增的人员（约100条）
        print(f'\n生成昨天新增的人员数据（{yesterday_start.strftime("%Y-%m-%d")}）...')
        for i in range(100):
            try:
                # 随机生成昨天的时间
                random_seconds = random.randint(0, int((yesterday_end - yesterday_start).total_seconds()))
                created_time = yesterday_start + timedelta(seconds=random_seconds)
                # 最后更新时间可能在昨天或今天
                if random.random() > 0.3:
                    # 70%的概率最后更新在昨天
                    last_update_time = created_time + timedelta(hours=random.randint(0, 23))
                else:
                    # 30%的概率最后更新在今天（但创建时间仍是昨天）
                    last_update_time = today + timedelta(hours=random.randint(0, now.hour))
                ...
                # 使用SQL直接插入，设置created_at为昨天
                # 注意：需要包含nationality字段（虽然前端不使用，但数据库字段存在）
                cursor = db.connection.cursor()
                cursor.execute('''
                    INSERT INTO people (
                        name, id_card, region, age, phone, status, avatar, last_update,
                ...
                ''', (
                    ...
                    person_data['visaType'],
                    person_data['institution'],
                    created_time.strftime('%Y-%m-%d %H:%M:%S')  # 设置创建时间为昨天
                ))
                db.connection.commit()
                added_count += 1
                
                if added_count % 20 == 0:
                    print(f'已添加 {added_count} 条数据（今天+昨天）...')
                    
            except Exception as e:
                print(f'添加第 {i+1} 条昨天数据失败: {e}')
                import traceback
                traceback.print_exc()
                continue
